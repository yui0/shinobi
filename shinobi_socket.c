...
#include "getopt_win.h"	// for LOG_ERR
#else
...
int send_connect_req(shinostat_t *stedstat)
{
	static fd_set fds;
	...
	if (ssl_write(&stedstat->sock, connect_req, strlen(connect_req)) < 0) {
	...
	if (FD_ISSET(sock, &fds)) {
		...
		if ((cnt = ssl_read(&stedstat->sock, buf, SOCKBUFSIZE)) < 0) {
			SET_ERRNO();
...
int open_socket(shinostat_t *stedstat, char *hub, char *proxy)
{
	...
	if (!(hub_name = strtok(hub, ":"))) {
		print_err(LOG_ERR, "hub name was not given\n");
		return(-1);
	}
	if (!(hub_port_string = strtok(NULL, ":"))) {
		hub_port = PORT_NO;
	} else {
	...
	if (!proxy) {
		...
		if (!(proxy_name = strtok(proxy, ":"))) {
			print_err(LOG_ERR, "proxy name was not given\n");
			return(-1);
		}
		if (!(proxy_port_string = strtok(NULL, ":"))) {
			proxy_port = PORT_NO;
		} else {
			proxy_port = atoi(proxy_port_string);
		}
		if (!(hp = gethostbyname(proxy_name))) {
			print_err(LOG_ERR, "hostname %s not found.\n", proxy_name);
			return(-1);
		}
	...
	if (connect(sock, (struct sockaddr *)&sin, sizeof sin) < 0) {
		SET_ERRNO();
		print_err(LOG_ERR, "connect to %s:%d: fd%d, %s(%d)\n", inet_ntoa(sin.sin_addr), sin.sin_port, sock, strerror(errno), errno);
		return(-1);
	}

	// stedstat 構造体に FD を記録
	stedstat->sock.fd = sock;
	ssl_open(&stedstat->sock, 1);
	...
	if (fcntl(sock, F_SETFL, O_NONBLOCK) == -1) {
		SET_ERRNO();
		print_err(LOG_ERR, "Failed to set nonblock: %d, %s\n", sock, strerror(errno));
		return(-1);
	}
#endif

	// HUB 経由の場合 CONNECT リクエストを作成。
	if (proxy != NULL) {
		int stat;
...
u_char *read_socket_header(shinostat_t *stedstat, int *cnt, u_char *readp)
{
...
int read_socket(shinostat_t *stedstat)
{
	...
	if ((recvsize = ssl_read(&stedstat->sock, recvbuf, SOCKBUFSIZE)) < 0) {
		SET_ERRNO();
		if (errno == EINTR || errno == EWOULDBLOCK || errno == 0) {
			if (debuglevel > 1) {
	...
	if (debuglevel > 1) {
		...
		if (debuglevel > 3) {
			int n = sizeof(stehead_t);
			print_err(LOG_DEBUG, "[%d.%d.%d.%d]->[%d.%d.%d.%d]\n", recvbuf[n+26], recvbuf[n+27], recvbuf[n+28], recvbuf[n+29], recvbuf[n+30], recvbuf[n+31], recvbuf[n+32], recvbuf[n+33]);
		}
		if (debuglevel > 2) {
			int i;
	...
	cnt = recvsize;		// 未処理データサイズをセット
	readp = recvbuf;	// 処理用のポインタをセット
	while (1) {
		if (stedstat->dataleft == 0) {
			// 受信データの先頭部分。先頭部分は stehead なので、
			// 読み込んで、フレームのサイズを得る。
			u_char *newp;
			if (!(newp = read_socket_header(stedstat, &cnt, readp))) {
...
//---------------------------------------------------------
//	sendbuf に溜まっているデータを仮想 HUB へ転送する
//---------------------------------------------------------
//  引数：
//           stedstat   : sted 管理用構造体
...
int write_socket(shinostat_t *stedstat)
{
	...
	//if (send(stedstat->sock.fd, stedstat->sendbuf, stedstat->sendbuflen, 0) < 0) {
	if (ssl_write(&stedstat->sock, stedstat->sendbuf, stedstat->sendbuflen) < 0) {
		SET_ERRNO();
		if (errno == EINTR || errno == EWOULDBLOCK || errno == 0) {
			if (debuglevel > 1) {
	...
	stedstat->sendbuflen = 0;  /* 書き込みサイズを 0 に戻す */
	if (debuglevel > 1) {
		print_err(LOG_DEBUG, "write_socket returned\n");
	}
	return(0);
